...
var validIMPlatforms = map[string]bool{
	...
	"wechat": true, "wechatkf": true, "qqbot": true,
}
...
func (h *IMHandler) CreateIMChannel(c *gin.Context) {
	...
	if !validIMPlatforms[req.Platform] {
		c.JSON(http.StatusBadRequest, gin.H{"error": "platform must be 'wecom', 'feishu', 'slack', 'telegram', 'dingtalk', 'mattermost', 'wechat', 'wechatkf' or 'qqbot'"})
		return
	}
	...
	} else if req.Platform == "wechatkf" {
		// WeChat KF uses webhook mode and full output only
